...
class PrepareSubnetExecutor(object):
    ...
    def execute(self, subnet_actions, reservation, aws_ec2_datamodel, cancellation_context, logger, ec2_session, ec2_client):
        ...
        availability_zone = self.vpc_service.get_or_pick_availability_zone(ec2_client, vpc, aws_ec2_datamodel)

        # get existing subnet bt their cidr
        for item in action_items:
            self._step_get_existing_subnet(item, logger, cancellation_context, vpc)

        # create new subnet for the non-existing ones
        for item in action_items:
            self._step_create_new_subnet_if_needed(item, logger, cancellation_context, vpc, availability_zone)

        # wait for the new ones to be available
        for item in action_items:
            self._step_wait_till_available(item, logger, cancellation_context)

        # set tags
        for item in action_items:
            self._step_set_tags(item, logger, cancellation_context, reservation)

        # set non-public subnets with private route table
        for item in action_items:
            self._step_attach_to_private_route_table(item, logger, cancellation_context, vpc, reservation, ec2_session, ec2_client)
    ...
    def step_wrapper(step):
        def wrapper(self, item, logger, cancellation_context, *args, **kwargs):
            self.cancellation_service.check_if_cancelled(cancellation_context)
            if item.error:
                return
            try:
                step(self, item, logger, cancellation_context, *args, **kwargs)
    ...
    @step_wrapper
    def _step_get_existing_subnet(self, item, logger, cancellation_context, vpc):
    ...
    @step_wrapper
    def _step_create_new_subnet_if_needed(self, item, logger, cancellation_context, vpc, availability_zone):
        if not item.subnet:
            cidr = item.action.connection_params.cidr
    ...
    @step_wrapper
    def _step_wait_till_available(self, item, logger, cancellation_context,):
        if item.is_new_subnet:
            logger.info("Waiting for subnet {0} - start".format(item.action.connection_params.cidr))
            self.subnet_waiter.wait(item.subnet, self.subnet_waiter.AVAILABLE)
            logger.info("Waiting for subnet {0} - end".format(item.action.connection_params.cidr))

    @step_wrapper
    def _step_set_tags(self, item, logger, cancellation_context, reservation):
    ...
    @step_wrapper
    def _step_attach_to_private_route_table(self, item, logger, cancellation_context, vpc, reservation, ec2_session, ec2_client):
