...
class TestAWSShell(TestCase):
    ...
    def test_deploying_ami_returns_deploy_result(self):
        ...
        result = self.aws_shell_api.deploy_ami(deploymock, aws_cloud_provider)
        self.assertEqual(jsonpickle.decode(result)['vm_name'], name)
        self.assertEqual(jsonpickle.decode(result)['vm_uuid'], result.instance_id)
        self.assertEqual(jsonpickle.decode(result)['auto_power_on'], deploymock.auto_power_on)
        self.assertEqual(jsonpickle.decode(result)['auto_power_off'], deploymock.auto_power_off)
        self.assertEqual(jsonpickle.decode(result)['wait_for_ip'], deploymock.wait_for_ip)
        self.assertEqual(jsonpickle.decode(result)['auto_delete'], deploymock.auto_delete)
        self.assertEqual(jsonpickle.decode(result)['autoload'], deploymock.autoload)
        self.assertEqual(jsonpickle.decode(result)['cloud_provider_resource_name'],deploymock.aws_ec2)
